...
class PersonneService {
  ...
  getAll(): Promise<Array<Personne>> {
    return new Promise((resolve) => {
      resolve(this.personnes);
    });
  }
  removeByNom(nom: string) {
    // Keep only personne that don't have nom as key
    this.personnes = this.personnes.filter(
      (personne: Personne) => personne.nom != nom
    );
  }
  removeByAttribut(nomAttibut: string, attributValue: boolean) {
    if (nomAttibut == "blond") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.blond == attributValue
      );
    } else if (nomAttibut == "brun") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.brun == attributValue
      );
    } else if (nomAttibut == "roux") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.roux == attributValue
      );
    } else if (nomAttibut == "lunettes") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.lunettes == attributValue
      );
    } else if (nomAttibut == "barbe") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.barbe == attributValue
      );
    } else if (nomAttibut == "sexeH") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.sexeH == attributValue
      );
    } else if (nomAttibut == "sexeF") {
      this.personnes = this.personnes.filter(
        (personne: Personne) => personne.sexeF == attributValue
      );
    }
  }
}
